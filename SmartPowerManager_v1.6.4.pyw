...
class ScheduleManager:
    ...
    def save(self):
        ...
        data = {
            ...
            "pico_settings": self.pico_settings, # v1.6.0 save
...
class SmartPowerManagerApp(tk.Tk):
    ...
    def _setup_update_tab(self):
        ...
        ttk.Label(frame, text="📦 自動アップデート", font=("Meiryo UI", 14, "bold")).pack(pady=(0, 20))
        ttk.Label(frame, text=f"現在のバージョン: v{APP_VERSION}", font=("Meiryo UI", 11)).pack(pady=5)
    ...
    def _update_tree(self, q_tree, w_tree, o_tree, action):
        ...
        for s in self.schedule_manager.onetime_schedules:
            ...
            try:
                ...
                if source == "quick":
                     if not s.get("executed") and dt <= tomorrow.replace(hour=23,minute=59):
