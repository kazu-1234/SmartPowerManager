...
def get_mac_addresses():
    ...
    if not mac_list:
        try:
            ...
            mac_str = ':'.join(f'{(mac_int >> i) & 0xff:02x}' 
...
class ScheduleManager:
    ...
    def __init__(self, config_path=CONFIG_FILE):
        ...
        # 毎週スケジュール: 各曜日ごとに個別設定
        # weekday: 0=月曜日, 6=日曜日
        self.weekly_schedules = []  # [{"id": uuid, "weekday": 0-6, "hour": 23, "minute": 0}]
    ...
    def get_next_shutdown_info(self):
        """
        次のシャットダウン予定を取得
        Returns: (datetime, type_str, id) or (None, None, None)
        """
        now = datetime.now()
        candidates = []
